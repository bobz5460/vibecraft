...
### 2026-07-19: Minecraft 26.2 environment-lighting alignment
- Owner: Codex
- Scope: Replace the scalar terrain-light approximation with the 26.2 RGB lightmap math, align the Overworld sky-light clock track, retain cardinal face/AO shading, and use linear reference fog interpolation. Full biome environment attributes, weather modifiers, sunrise/sunset keyframes, moon phases, and per-model `shade`/emission propagation remain follow-up work.
- Depends on: packed sky/block light and current renderer uniform path
- Acceptance: Day, night, and torch-lit terrain combine independently curved and colored sky/block light without camera-relative terrain shading; startup validates the changed WGSL pipeline with 26.2 assets.
- Status: partial
- Evidence: `cargo build` succeeds; `timeout 15 cargo run --release` reaches the healthy windowed runtime and exits by timeout without a panic or wgpu validation error.
...
- 2026-07-19 frame-pacing investigation: saved-chunk reads/JSON decoding, generated-chunk encoding/fsync, and streaming-eviction writes moved off the render thread; the 30-second autosave now queues chunk snapshots instead of waiting for every chunk write. Worker-result admission/GPU upload is frame-budgeted and background generation/meshing reserves two logical CPUs for the client and driver. Minecraft 26.2 uses asynchronous `IOWorker` writes, background encoding, priority chunk dispatch, a shared `availableProcessors - 1` executor, and elevates the client thread; Vibecraft now preserves the equivalent ownership constraints without claiming equivalent section occlusion or renderer throughput. Full in-world capture remains required before closing the occlusion/LOD item.
- 2026-07-19 player-centered streaming correction: chunk arrival/removal no longer invalidates the one global lighting epoch. Derived light/mesh work now has per-chunk topology revisions, so only jobs whose actual 3x3 inputs changed are rejected. Requested chunks wait for requested generation neighbors before lighting, light batches are small and deterministically nearest-first, and the non-reprioritizable generation channel is limited to one submitted task per worker with obsolete tasks retained until their real result arrives. This mirrors 26.2's distance-ticket priority, dependency/status neighborhoods, task resorting, and queued-task release at the native architecture boundary; it does not add Java's full `ChunkStatus` graph.
- 2026-07-19 initial-load regression correction: a valid lighting flood now retires every dirty key in its expanded scope instead of repeatedly relighting overlapping 3x3 regions. World entry waits for the player's uploaded chunk rather than 9 existing-player or 25 new-player full-height column meshes, matching 26.2's client-side compiled-player-section gate while neighbor columns continue streaming. Initial spawn selection uses the source's fixed 11x11 chunk spiral and scans each chunk's columns together; exhaustion retains a surface-adjusted preliminary suggestion rather than failing world creation. Selecting a new spawn queues generated chunk saves instead of synchronously waiting for every JSON/fsync.
- 2026-07-19 progressive-neighbor rendering correction: first mesh publication now requires requested geometry neighbors and the chunk's own accepted lighting, but no longer requires all eight neighbors to remain simultaneously light-clean. Later overlapping light results mark every mesh in their 3x3 sampling neighborhood dirty and replace those meshes after publication instead of invalidating/withholding the first visible result. This follows 26.2's progressive section compilation and dirty-section rebuild behavior within Vibecraft's column-mesh architecture.
- 2026-07-19 lost-first-mesh correction: `has_mesh` and active worker ownership are now the authoritative first-mesh state; the dirty-key set is only a scheduling index and is rederived for any loaded, lit chunk with neither a mesh nor a task. Dispatch no longer clears `Chunk::is_dirty`; successful publication clears it only when no newer dirty event arrived during the task. The reported Test4 chunk `(12,21)` was verified from its exact saved 3x3 neighborhood to produce 8,292 opaque and 2,284 translucent vertices, confirming queue-state loss rather than empty geometry. Dropped-item construction also rejects invalid IDs/counts/durability before entities reach persistence.
- 2026-07-19 chunk-border convergence correction: neighbor/topology and lighting invalidations that occur while a mesh task is active now retain their replacement request instead of losing it when the scheduler drains the dirty queue. This prevents a pre-neighbor snapshot from permanently preserving fallback-air lighting or geometry. Fluid sides also follow 26.2 `FluidRenderer::shouldRenderFace`: adjacent fluids of the same type suppress their internal face regardless of flow level, with cross-chunk regression coverage.
- 2026-07-19 high-distance throughput correction: stable oceans/lava are no longer globally scanned on periodic simulation ticks. Block changes schedule bounded position-specific fluid work for the affected cell and neighbors, matching 26.2's `LiquidBlock`/`FlowingFluid` scheduled-tick ownership; loaded non-source fluid states are reactivated for native saves that cannot yet persist scheduled ticks. Completed mesh admission rises from two to eight columns per frame and uploads `MeshVertex` storage directly, removing two full vertex-array allocation/copy passes per column. Minecraft's shared terrain-buffer allocator, grouped draws, section-level compilation, and occlusion graph remain the principal known gaps for 32-chunk steady-state FPS.
- 2026-07-19 recursive generation profile: a release probe measured 26.2 Geometry generation at ~6.95 seconds/16 chunks and attributed about 20% of CPU to Vibecraft's linear scan of all 7,594 climate points. Porting 26.2's seven-dimensional `Climate.RTree` reduced the same probe to ~4.33 seconds/16 chunks (~38%). Generation workers now share one immutable noise router/climate index instead of rebuilding one per worker. Initial chunk arrival no longer relights/remeshes unready neighbors that had no derived result to invalidate, and exact-coordinate density cache markers now memoize within the reference cell without changing fixed output. Remaining generation work is a full shared `NoiseChunk` cache/interpolator lifecycle; remaining render work is section meshes, occlusion, shared terrain buffers, and grouped draws.
- 2026-07-19 climate tie regression: The first R-tree port retained nearest fitness but discarded the source parameter-list ordinal. At seed 1, block 172/96/182 is exactly equidistant from Forest and Beach; arbitrary tree traversal selected Beach and removed the reference trees/greenery. R-tree leaves and branches now retain their minimum source ordinal, equal-distance branches remain searchable only when they can improve that ordinal, and equal leaves select the earliest 26.2 parameter entry. The release reproduction now selects Forest and generates 574 oak/birch log/leaf blocks in chunk 10/11 while preserving indexed pruning.
...
- 2026-07-17 correction: Geometry feature height placement now uses the completed target chunk's ocean floor rather than the router's coarse preliminary-surface estimate. The old lookup placed seed-1 forest trees near Y=56 inside completed Y=68–70 terrain, so their air-only writes were discarded and the landscape appeared barren. Generated chunks are now persisted on admission, structure templates share the sibling 26.2 asset fallback, and fixed seed-1 density/preliminary-surface points are checked against the official in-process Java 26.2 oracle.
- 2026-07-17 placement-behavior correction: Native flower geometry now retains Java's outer placed-feature rarity/count boundary, clustered configured-patch attempts and trapezoid offsets, dirt-family survival, and representable flower providers instead of emitting 32 guaranteed dandelions for every flower root. Tree roots now use their supplied weighted count/rarity distributions (including zero-attempt plains, snowy, water, meadow, and windswept cases), and the seed-1 forest selector follows the ordered fallen-birch/birch/fancy-oak/fallen-oak/default-oak probabilities. Geometry remains approximate where configured noise providers, exact tree state/shape, leaf litter, double plants, or bee nests are not representable.
- 2026-07-19 vegetation/magma correction: Grass, tall-grass, fern, and mushroom roots now preserve 26.2's outer count/rarity modifiers and inner 7x3 triangular patch offsets instead of resampling the surface for dozens of guaranteed, uniformly distributed successes. The static seed-2345 biome-info noise drives the reference noise-threshold counts; normal mushroom rarity is restored from the erroneous 1/8 to brown 1/256 and red 1/512, with root-specific taiga/old-growth/swamp values and basic light/podzol survival. Meadow flowers use the supplied grass/flower state list; cherry pink-petal and forest double-flower roots are omitted instead of substituting dandelions for unavailable blocks. Underwater magma now requires the water column above its floor and cannot replace dry Stony Shore terrain.
- 2026-07-17 texture correction: Atlas registration is now closed over the canonical block-to-resource map, including generated spawners, magma and amethyst blocks. Partial vanilla models use a resolved face texture instead of requesting nonexistent base-name PNGs, chest's temporary cube uses its model-declared oak-plank material until block-entity geometry exists, and missing mappings are diagnosed once per block face rather than once per emitted face.
- 2026-07-17 ocean correction: Surface-lava lake writes now preserve Java LakeFeature's critical liquid-boundary/solid-floor rejection instead of carving through ocean water and sand. Kelp and seagrass use their real crossed textures while retaining a meshed waterlogged volume. Water animation stays within its atlas tile, and underwater fog uses a dark 26.2-derived color with a provisional half-range pending the vanilla water-vision timer and biome attribute interpolation.
- 2026-07-17 ocean/sky follow-up: The underwater-magma placed feature now consumes 26.2's omitted uniform height attempt and five-block floor-scan gate instead of placing all 44–52 attempts directly on the floor. Waterlogged plant adjacency is symmetric, animated water/lava/kelp strips retain and select real `.mcmeta`-timed frames, underwater water surfaces become nearly opaque, and the procedural cloud noise is removed. Sun/full-moon quads use the 26.2 celestial assets, Java's `OVERLAY` blend, Java's distances/scales, and fixed sky-clock axes rather than camera billboards. The repeating cloud mask is converted into Java-style 12×4×12 exposed-face geometry at Y=192, drifts at 0.6 blocks/second, and only rebuilds on cell/range/relative-height changes. A remaining camera-projected panoramic horizon gradient/orange band was traced against `SkyRenderer`: 26.2 instead fills its upper disc with one environment sky color and renders sunrise as a localized conditional fan, so both invented full-width terms were removed. Moon phases, environment/biome sky-color input, localized sunrise/sunset, weather opacity/color, exact cloud render-target composition, and the camera-inside-cell faces remain follow-up fidelity work.
- 2026-07-17 local-load recovery: Existing saves now make their acceptance chunks first-class generation priorities and the loading screen streams only that set plus its one-chunk lighting/mesh dependency halo. Normal render-distance streaming begins after entry, preventing hundreds of unrelated columns and continuing topology invalidations from starving saved-player terrain. Stage-count diagnostics are emitted at debug level during progress and warning level on a terminal stall.
